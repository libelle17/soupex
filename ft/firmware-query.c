...
gboolean fbcl::fritzbox_get_settings_query(/*struct profile *profile*/)
{
	JsonParser *parser;
	JsonReader *reader;
	SoupMessage *msg;
...
//	gchar *scramble;
  ...
  printf("Beginn fritzbox_get_settings_query\n");
	...
	g_debug("LKZ: %s", lkz);
	countrycode=lkz;//g_settings_set_string(profile->settings, "country-code", lkz);
	...
	internationalcallprefix=lkz_prefix;//g_settings_set_string(profile->settings, "international-call-prefix", lkz_prefix);
	json_reader_end_member(reader);

	json_reader_read_member(reader, "OKZ");
	const gchar *okz = json_reader_get_string_value(reader);
	g_debug("OKZ: %s", okz);
	areacode=okz;//g_settings_set_string(profile->settings, "area-code", okz);
	json_reader_end_member(reader);
	...
	nationalcallprefix=okz_prefix;//g_settings_set_string(profile->settings, "national-call-prefix", okz_prefix);
	json_reader_end_member(reader);
	...
	*/
	...
	faxident=fax_ident;//g_settings_set_string(profile->settings, "fax-header", fax_ident);
	json_reader_end_member(reader);
	...
	if ((port == 2 || port == 3)) {
		...
		faxvolume=storage;//g_settings_set_string(profile->settings, "fax-volume", storage);
		json_reader_end_member(reader);
	} else {
	...
	*/
	...
	for (i = 0; i < 3; i++) {
		...
		nameanalog=name;//g_settings_set_string(profile->settings, name_analog, name);
		json_reader_end_member(reader);
	}
	...
	for (i = 0; i < 8; i++) {
		...
		nameisdn=name;//g_settings_set_string(profile->settings, name_isdn, name);
		json_reader_end_member(reader);
	}
	...
	for (i = 1; i < count; i++) {
		...
		namedect=tmp;//g_settings_set_string(profile->settings, name_dect, tmp);
...
//	g_settings_set_strv(profile->settings, "numbers", (const gchar * const *)numbers);
	...
	if (tam_stick && atoi(&tam_stick[0])) {
		tamstick=atoi(tam_stick);//g_settings_set_int(profile->settings, "tam-stick", atoi(tam_stick));
	} else {
