...
struct router *active_router = NULL;
/** Global router plugin list */
GSList *router_list = NULL;
/** Router login blocked shield */
gboolean router_login_blocked = FALSE;
...
{
	struct phone *phone = (struct phone*)data;
 ...
 */
/*
gboolean fbcl::router_login(struct profile *profile)
{
	gboolean result;
 ...
 */
gboolean fbcl::router_dial_number(/*struct profile *profile, */gint port, const gchar *number)
{
	gboolean ret;
	/*
	...
	g_free(target);
	*/
	struct router_info ri={0};
  // ret=fritzbox_present(&ri);
	// fprintf("ret: %i\n",ret);
	exit(0);
 ...
 * \return phone port
 */
gint fbcl::router_get_phone_port(/*struct profile *profile*/)
{
	return port;//g_settings_get_int(profile->settings, "port");
}
 ...
 */
void fbcl::router_set_phone_port(/*struct profile *profile, */gint _port)
{
	port=_port;//g_settings_set_int(profile->settings, "port", _port);
}
