 ...
 */
void fbcl::fritzbox_detect_controller_06_35(/*struct profile *profile, */const gchar *data)
{
	gint index;
	gint type = 4;
...
gboolean fritzbox_get_fax_information_06_35(/*struct profile *profile*/)
{
  ...
  printf("Beginn fritzbox_get_fax_information_06_35\n");
	url = g_strdup_printf("http://%s/fon_devices/fax_option.lua", host.c_str()/*router_get_host(profile)*/);
	...
	while (match_info && g_match_info_matches(match_info)) {
		...
		if (name) {
			...
			g_free(scramble);
			*/
			faxheader=name;//g_settings_set_string(profile->settings, "fax-header", name);
			break;
		}
	...
	tamstick=store;//g_settings_set_int(profile->settings, "tam-stick", store);
...
//			gchar *scramble;
			...
			g_free(scramble);
			*/
			g_debug("Fax number: '%s'", msn);

			faxmsn=msn;//g_settings_set_string(profile->settings, "fax-number", msn);
...
void fbcl::fritzbox_extract_phone_names_06_35(/*struct profile *profile, */const gchar *data, gsize read)
{
		...
		if (port && name) {
			...
			for (index = 0; index < PORT_MAX; index++) {
				if (fritzbox_phone_ports[index].number == val) {
					...
					phoneports.push_back(string(router_phone_ports[index].name));//g_settings_set_string(profile->settings, router_phone_ports[index].name, name);
				}
			}
		}
 ...
 * \return error code
 */
gboolean fbcl::fritzbox_get_settings_06_35(/*struct profile *profile*/)
{
	SoupMessage *msg;
	const gchar *data;
	gsize read;
 ...
 */
gboolean fbcl::fritzbox_dial_number_06_35(/*struct profile *profile, */gint port, const gchar *number)
{
	SoupMessage *msg;
	gchar *port_str;
	gchar *url;
//	gchar *scramble;
	...
	g_free(scramble);
	*/
	g_debug("Call number '%s' on port %d...", number, current_port);
 ...
 * \return TRUE on success, otherwise FALSE
 */
gboolean fbcl::fritzbox_hangup_06_35(/*struct profile *profile*/, gint port, const gchar *number)
{
	SoupMessage *msg;
	gchar *port_str;
	gchar *url;
...
//	if (!router_login(profile)) {
	...
	g_free(scramble);
	*/
	g_debug("Hangup call '%s' on port %s...", number, port_str);

  printf("Beginn fritzbox_hangup_06_35\n");
	url = g_strdup_printf("http://%s/fon_num/foncalls_list.lua", host.c_str()/*router_get_host(profile)*/);
	msg = soup_form_request_new(SOUP_METHOD_GET, url,
	                            "sid", /*profile->router_info->*/session_id,
	                            "hangup", "",
	                            NULL);
