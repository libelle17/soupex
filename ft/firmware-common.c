...
gchar **xml_extract_tags(const gchar *data, gchar *tag_start, gchar *tag_end)
{
	...
	while (match_info && g_match_info_matches(match_info)) {
		...
		if (fetched == TRUE) {
			const gchar *tag_start_pos = strchr(data + start, '>');
			const gchar *tag_end_pos = strchr(tag_start_pos + 1, '<');
 ...
 */
gboolean fbcl::fritzbox_present(/*struct router_info *router_info*/)
{
	SoupMessage *msg;
	gsize read;
	const gchar *data;
  ...
  printf("Beginn fritzbox_present\n");
	_url = g_strdup_printf("http://%s/jason_boxinfo.xml", host.c_str()/*router_info->host*/);
	msg = soup_message_new(SOUP_METHOD_GET, _url);
#ifndef original 
...
//				SOUP_SESSION_ADD_FEATURE_BY_TYPE, SOUP_TYPE_COOKIE_JAR,
  ...
  g_object_unref (session);
	...
	log_save_data((gchar*)"fritzbox-present.html", data, read);
	g_return_val_if_fail(data != NULL, FALSE);

	_name = xml_extract_tag(data, (gchar*)"j:Name");
	_version = xml_extract_tag(data, (gchar*)"j:Version");
	_lang = xml_extract_tag(data, (gchar*)"j:Lang");
	_serial = xml_extract_tag(data, (gchar*)"j:Serial");
	_annex = xml_extract_tag(data, (gchar*)"j:Annex");

	g_object_unref(msg);
	g_free(_url);

	if (_name && _version && _lang && _serial && _annex) {
		gchar **split;

		/*router_info->*/name = g_strdup(_name);
		/*router_info->*/version = g_strdup(_version);
		/*router_info->*/lang = g_strdup(_lang);
		/*router_info->*/serial = g_strdup(_serial);
		/*router_info->*/annex = g_strdup(_annex);
 ...
 * \return error code
 */
gboolean fbcl::fritzbox_logout(/*struct profile *profile, */gboolean force)
{
	SoupMessage *msg;
	gchar *url;
  ...
  printf("Beginn fritzbox_logout\n");
	url = g_strdup_printf("http://%s/home/home.lua", host.c_str()/*router_get_host(profile)*/);
	msg = soup_form_request_new(SOUP_METHOD_POST, url,
	                            "sid", /*profile->router_info->*/session_id,
	                            "logout", "1",
	                            NULL);
...
void fritzbox_read_msn(/*struct profile *profile, */const gchar *data)
{
	...
	pots = (gchar*)g_slice_alloc0(pots_len + 1);
	...
	for (index = 0; index < 10; index++) {
		...
		msns = (gchar*)g_slice_alloc0(msns_len + 1);
	...
	for (index = 0; index < 19; index++) {
		...
		sips = (gchar*)g_slice_alloc0(sips_len + 1);
 ...
 */
/*
GSList *fbcl::fritzbox_load_faxbox(GSList *journal)
{
...
//	gchar *user = router_get_ftp_user(profile);
  ...
  printf("Beginn fritzbox_load_faxbox\n");
	client = ftp_init(host.c_str()*//*router_get_host(profile)*//*);
	if (!client) {
		return journal;
	}

	if (!ftp_login(client, usr.c_str(), pwd.c_str()*//*router_get_ftp_password(profile)*//*)) {
	...
	volume_path = faxpfad*//*g_settings_get_string(profile->settings, "fax-volume")*//*;
	...
	ftp_shutdown(client);
  printf("Ende fritzbox_load_faxbox\n");
...
GSList *fritzbox_load_voicebox(GSList *journal)
{
  ...
  printf("Beginn fritzbox_load_voicebox\n");
	...
	ftp_shutdown(client);
  printf("Ende fritzbox_load_voicebox\n");
 ...
 * \return fax data
 */
gchar *fbcl::fritzbox_load_fax(/*struct profile *profile, */const gchar *filename, gsize *len)
{
	struct ftp *client;
	gchar *user = (gchar*)usr.c_str()/*router_get_ftp_user(profile)*/;
	gchar *ret;

  printf("Beginn fritzbox_load_fax\n");
	client = ftp_init(host.c_str()/*router_get_host(profile)*/);
	ftp_login(client, user, (gchar*)pwd.c_str()/*router_get_ftp_password(profile)*/);
 ...
 */
gchar *fbcl::fritzbox_get_ip(/*struct profile *profile*/)
{
	SoupMessage *msg;
	SoupURI *uri;
	gchar *ip = NULL;
  ...
  printf("Beginn fritzbox_get_ip\n");
	if (FIRMWARE_IS(6, 6)) {
		url = g_strdup_printf("http://%s/igdupnp/control/WANIPConn1", host.c_str()/*router_get_host(profile)*/);
	} else {
		url = g_strdup_printf("http://%s/upnp/control/WANIPConn1", host.c_str()/*router_get_host(profile)*/);
	}
 ...
 */
gboolean fbcl::fritzbox_reconnect(/*struct profile *profile*/)
{
	SoupMessage *msg;
	SoupURI *uri;
	gchar *request;
  ...
  printf("Beginn fritzbox_reconnect\n");
	if (FIRMWARE_IS(6, 6)) {
		url = g_strdup_printf("http://%s:49000/igdupnp/control/WANIPConn1", host.c_str()/*router_get_host(profile)*/);
	} else {
		url = g_strdup_printf("http://%s:49000/upnp/control/WANIPConn1", host.c_str()/*router_get_host(profile)*/);
	}
 ...
 * \return TRUE on success, otherwise FALSE
 */
gboolean fbcl::fritzbox_delete_fax(/*struct profile *profile, */const gchar *filename)
{
	struct ftp *client;
	gchar *user = (gchar*)usr.c_str()/*router_get_ftp_user(profile)*/;
	gboolean ret;
  ...
  printf("Beginn fritzbox_delete_fax\n");
	client = ftp_init(host.c_str()/*router_get_host(profile)*/);
	ftp_login(client, user, pwd.c_str()/*router_get_ftp_password(profile)*/);
