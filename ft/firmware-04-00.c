 ...
 */
gboolean fbcl::fritzbox_present_04_00(/*struct router_info *router_info*/)
{
	SoupMessage *msg;
	const gchar *data;
	gchar *url;
  ...
  printf("Beginn fritzbox_present_04_00\n");
	url = g_strdup_printf("http://%s/cgi-bin/webcm", host.c_str()/*router_info->host*/);
	...
	log_save_data((gchar*)"fritzbox-04_00-present.html", data, read);
	g_assert(data != NULL);

	if (/*g_*/strcasestr(data, "fritz!box")) {
		ret = TRUE;
...
gboolean fbcl::fritzbox_login_04_00(/*struct profile *profile, */)
{
	SoupMessage *msg;
	const gchar *data;
	gchar *url;
  ...
  printf("Beginn fritzbox_login_04_00\n");
	...
	password = (gchar*)pwd.c_str(); // router_get_login_password(profile);
	...
	log_save_data((gchar*)"fritzbox-04_00-login1.html", data, read);
	g_assert(data != NULL);
 ...
 */
gboolean fbcl::fritzbox_dial_number_04_00(/*struct profile *profile, */gint port, const gchar *number)
{
	SoupMessage *msg;
	gchar *port_str;
	//gchar *scramble;
  ...
  printf("Beginn fritzbox_dial_number_04_00\n");
	gchar *url = g_strdup_printf("http://%s/cgi-bin/webcm", host.c_str()/*router_get_host(profile)*/);
	...
	//g_free(scramble);
	g_debug("Call number '%s' on port %s...", number, port_str);
 ...
 * \return TRUE on success, otherwise FALSE
 */
gboolean fbcl::fritzbox_hangup_04_00(/*struct profile *profile, */gint port, const gchar *number)
{
	SoupMessage *msg;
	gchar *port_str;
