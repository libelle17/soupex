 ...
 * \return error code
 */
gboolean fbcl::fritzbox_login_04_74(/*struct profile *profile*/)
{
	SoupMessage *msg;
	gchar *response = NULL;
	gsize read;
 ...
 */
gint fbcl::number_compare_04_74(gconstpointer a, gconstpointer b)
{
	return strcmp(a, b);
}
 ...
 */
gboolean fbcl::extract_number_04_74(GSList **number_list, const gchar *data, gchar *msn_str)
{
	gchar *fon;
 ...
 * \return TRUE on success, otherwise FALSE
 */
gboolean fbcl::copy_number_04_74(GSList **number_list, const gchar *data, gsize len)
{
	gchar *fon;
...
void fritzbox_extract_numbers_04_74(/*struct profile *profile, */const gchar *data)
{
	...
	for (list = number_list; list; list = list->next) {
		...
		g_free(scramble);
		*/
		g_debug("Adding MSN '%s'", list->data);
		// numbers[counter++] = g_strdup(list->data);
    numbers.push_back(string(list->data));
	}
	//numbers[counter] = NULL;
//	g_settings_set_strv(profile->settings, "numbers", (const gchar * const *)_numbers);
 ...
 * \return error code
 */
gboolean fbcl::fritzbox_get_settings_04_74(/*struct profile *profile*/)
{
	SoupMessage *msg;
	const gchar *data;
	gint index;
  ...
  printf("Beginn fritzbox_get_settings_04_74\n");
	...
	for (index = 0; index < PORT_MAX; index++) {
		...
		if (value != NULL && strlen(value) > 0) {
			...
			phoneports.push_back(string(phone_ports[index].name));//g_settings_set_string(profile->settings, router_phone_ports[index].name, value);
		}
		g_free(value);
	}
	g_object_unref(msg);
	...
	if (fax_msn) {
		...
		*/
		...
		faxident=formatted_number;//g_settings_set_string(profile->settings, "fax-ident", formated_number);
	...
	tamstick=1;//g_settings_set_int(profile->settings, "tam-stick", 1);
 ...
 */
/*
void fbcl::fritzbox_journal_04_74_cb(SoupSession *session, SoupMessage *msg, gpointer user_data)
{
	GSList *journal = NULL;
 ...
 */
/*
gboolean fbcl::fritzbox_load_journal_04_74(struct profile *profile, gchar **data_ptr)
{
	SoupMessage *msg;
	gchar *url;
 ...
 */
/*
gboolean fbcl::fritzbox_clear_journal_04_74(struct profile *profile)
{
	SoupMessage *msg;
	gchar *url;
