...
static gboolean fbcl::fritzbox_check_login_blocked(const gchar *data/*, struct profile *profile*/)
{
	gboolean result;
 ...
 * \return error code
 */
gboolean fbcl::fritzbox_login_05_50(/*struct profile *profile*/)
{
  ...
  printf("Beginn fritzbox_login_05_50\n");
	url = g_strdup_printf("http://%s/login_sid.lua", host.c_str()/*router_get_host(profile)*/);
	...
	if (!strcmp(/*profile->router_info->*/session_id, "0000000000000000")) {
		const gchar *user=usr.c_str(); // router_get_login_user(profile);
		const gchar *password=pwd.c_str(); // router_get_login_password(profile);
 ...
 */
void fbcl::fritzbox_detect_controller_05_50(/*struct profile *profile, */const gchar *data)
{
	gint index;
	gint type = -1;
	gint port;
...
set:
	...
	controller=type;//g_settings_set_int(profile->settings, "fax-controller", type);
 ...
 * \param data incoming page data
 */
static void fbcl::fritzbox_extract_dect_05_50(/*struct profile *profile, */const gchar *data)
{
	...
	do {
		...
		g_debug("fon: '%s'", fon);
		namedect=fon;//g_settings_set_string(profile->settings, name_dect, fon);
		g_slice_free1(size, fon);
		count++;
	} while (count < 7);
}

gboolean fbcl::fritzbox_get_fax_information_05_50(/*struct profile *profile*/)
{
	SoupMessage *msg;
	const gchar *data;
	gsize read;
...
//	gchar *scramble;
  ...
  printf("Beginn fritzbox_get_fax_information_05_50\n");
	url = g_strdup_printf("http://%s/cgi-bin/webcm", host.c_str()/*router_get_host(profile)*/);
	msg = soup_form_request_new(SOUP_METHOD_POST, url,
	...
	if (header) {
		...
		*/
		...
		faxheader=header;// g_settings_set_string(profile->settings, "fax-header", header);
		g_free(header);
	}
...
//			gchar **numbers = g_settings_get_strv(profile->settings, "numbers");
			g_free(fax_msn);
			fax_msn = numbers[0].c_str()/*g_strdup(numbers[0])*/;
		}
		...
		g_free(scramble);
		*/
		g_debug("Fax number: '%s'", fax_msn);

		faxmsn=fax_msn;//g_settings_set_string(profile->settings, "fax-number", fax_msn);

		faxident=formated_number;//g_settings_set_string(profile->settings, "fax-ident", formated_number);
		g_free(formated_number);
	}
	g_free(fax_msn);

	faxvolume.clear();//g_settings_set_string(profile->settings, "fax-volume", "");
	...
	if (active) {
		...
		if ((fax_mail_active == 2 || fax_mail_active == 3)) {
			...
			if (volume) {
				...
				faxvolume=volume;//g_settings_set_string(profile->settings, "fax-volume", volume);
			} else {
				faxvolume.clear();//g_settings_set_string(profile->settings, "fax-volume", "");
			}
...
gboolean fbcl::fritzbox_get_fax_information_06_00(/*struct profile *profile*/)
{
	SoupMessage *msg;
	const gchar *data;
	gsize read;
...
//	gchar *scramble;
  ...
  printf("Beginn fritzbox_get_fax_information_06_00\n");
	url = g_strdup_printf("http://%s/fon_devices/fax_send.lua", host.c_str()/*router_get_host(profile)*/);
...
//			gchar **numbers = g_settings_get_strv(profile->settings, "numbers");
	...
	if (mail_active) {
		...
		if (fax_mail_active == 3) {
			...
			url = g_strdup_printf("http://%s/usb/show_usb_devices.lua", host.c_str()/*router_get_host(profile)*/);
			msg = soup_form_request_new(SOUP_METHOD_GET, url,
				                    "sid", /*profile->router_info->*/session_id,
				                    NULL);
			g_free(url);
 ...
 * \return error code
 */
gboolean fbcl::fritzbox_get_settings_05_50(/*struct profile *profile*/)
{
	SoupMessage *msg;
	const gchar *data;
	gint index;
...
//	if (!router_login(profile)) {
  ...
  printf("Beginn fritzbox_get_settings_05_50\n");
	url = g_strdup_printf("http://%s/fon_num/fon_num_list.lua", host.c_str()/*router_get_host(profile)*/);
	msg = soup_form_request_new(SOUP_METHOD_GET, url,
	                            "sid", /*profile->router_info->*/session_id,
	                            NULL);
	g_free(url);
	...
	gchar **_numbers = xml_extract_tags(data, "td title=\"[^\"]*\"", "td");

	if (g_strv_length(_numbers)) {
		gchar **profile_numbers = strv_remove_duplicates(_numbers);
		gint idx;
		...
		if (g_strv_length(profile_numbers)) {
			for (idx = 0; idx < g_strv_length(profile_numbers); idx++) {
				...
				g_free(scramble);
				*/
				numbers.push_back(string(profile_numbers[idx]);
			}
//			g_settings_set_strv(profile->settings, "numbers", (const gchar * const *)profile_numbers);
	...
	url = g_strdup_printf("http://%s/fon_devices/fondevices_list.lua", host.c_str()/*router_get_host(profile)*/);
	msg = soup_form_request_new(SOUP_METHOD_GET, url,
	                            "sid", /*profile->router_info->*/session_id,
	                            NULL);
	g_free(url);
	...
	for (index = 0; index < PORT_MAX; index++) {
		...
		if (value) {
			...
			phoneports.push_back(string(router_phone_ports[index].name));//g_settings_set_string(profile->settings, router_phone_ports[index].name, value);
			g_free(value);
		}
	}
	...
	if (stick && atoi(&stick[0])) {
		tamstick=atoi(stick);//g_settings_set_int(profile->settings, "tam-stick", atoi(stick));
	} else {
		tamstick=0;//g_settings_set_int(profile->settings, "tam-stick", 0);
	}
	g_free(stick);
	...
	url = g_strdup_printf("http://%s/fon_num/sip_option.lua", host.c_str()/*router_get_host(profile)*/);
	msg = soup_form_request_new(SOUP_METHOD_GET, url,
	                            "sid", /*profile->router_info->*/session_id,
	                            NULL);
	g_free(url);
	...
	countrycode=value;//g_settings_set_string(profile->settings, "country-code", value);
	g_free(value);
	...
	internationalcallprefix=value;//g_settings_set_string(profile->settings, "international-call-prefix", value);
	g_free(value);
	...
	areacode=value;//g_settings_set_string(profile->settings, "area-code", value);
	g_free(value);
	...
	nationalcallprefix=value;//g_settings_set_string(profile->settings, "national-call-prefix", value);
	g_free(value);
	...
	url = g_strdup_printf("http://%s/fon_num/dial_foncalls.lua", host.c_str()/*router_get_host(profile)*/);
	msg = soup_form_request_new(SOUP_METHOD_GET, url,
	                            "sid", /*profile->router_info->*/session_id,
	                            NULL);
	g_free(url);
